...
\title{Observers and confidence in an eternally inflating universe}
...
We consider an axionic inflection point inflationary model in which, for a certain parameter range, eternal inflation is guaranteed by topological eternal inflation.  Alternatively, when the parameter
is outside of this range, we prove eternal inflation is
impossible for any initial conditions.   Because the true parameter value is (in principle) measurable via the tensor-to-scalar ratio, the question ``is there a multiverse?'' is answerable in the context of the model.
In the no-eternal-inflation case, the fine-tunings of the parameter values and initial conditions are strongly correlated with the total number of e-foldings.  This suggests an anthropic resolution of the fine-tunings without encountering the conceptual problems caused by eternal inflation.  If eternal inflation is incompatible \cite{Obied:2018sgi, Brahma:2019iyy,Kinney:2018kew} with quantum gravity, 
such seemingly fine-tuned inflationary models stand out as islands in the swampland.
Otherwise the fine-tunings should be interpreted as evidence in favor of eternal inflation, since they are to a large extent absent in this case.
Then the fine-tuned initial conditions problem is replaced with the measure problem and the nature of observers.  Selection effects become possible when the number of thermalized regions is large, and Bayesian parameter estimation requires assumptions
about our place in the multiverse, even though the model has a unique vacuum (no landscape).  
In the context of this model, the existence of eternal inflation can be confirmed empirically if future data from the Cosmic Microwave Background measures the tensor-to-scalar ratio $r > 10^{-6}$. 
